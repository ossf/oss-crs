...
| `required_inputs` | `list[string]` | No | Input channels this CRS requires to function. Valid names: `diff`, `pov`, `seed`, `bug-candidate`, `report`. When declared, `oss-crs run` validates that the corresponding CLI flags were provided before spawning containers. |
