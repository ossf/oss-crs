...
3. **Export (optional)** — Bundle prepared Docker images and CRS source for transfer to another host (`oss-crs export`). Specify the CRS with --compose-file.
4. **Import (on another host)** — Load the exported Docker images and restore CRS source (`oss-crs import`). Specify the CRS with --compose-file.
5. **Run** — Launch all CRSs and shared infrastructure via Docker Compose (`oss-crs run`). Pass `--incremental-build` to use snapshot images for ephemeral rebuild containers. Pass `--web-ui` to monitor the run live in the [Web Dashboard](#web-dashboard).
