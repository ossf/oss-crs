...
- `oss-crs run --continue-on-container-exit` — keep a run going when a single container exits instead of tearing down the whole compose. For multi-CRS ensembles, where one CRS finishing (a seed generator completing, a fuzzer dying, triage returning) must not stop the others; the run ends at `--timeout` instead. Default is unchanged: abort on the first container exit.
- `oss-crs gen-compose --litellm-external-values URL KEY` — set litellm to external mode with literal URL and API key values instead of env var names. Mutually exclusive with `--litellm-external`. Lets a caller that mints per-run keys (e.g. CRSBench trial keys) pass them directly rather than exporting them into the environment.
