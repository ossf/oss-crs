...
### Added
- `oss-crs run --forward-artifacts <run-id>[,<run-id>...]` — forwards exchange artifacts from previous runs into the new run's `FETCH_DIR`. Resolution searches sibling compose-hash workdirs, prompts when a run ID maps to multiple source target/harness/sanitizer combinations, prefers processed POVs/seeds when available, and records provenance in `FORWARDED_ARTIFACTS.json`.
- Generated run input flags for exchange artifacts: `--pov/--pov-dir`, `--seed-dir`, `--bug-candidate/--bug-candidate-dir`, `--report/--report-dir`, and `--patch/--patch-dir`. `--diff` remains the special delta-mode reference diff input.
- `libCRS fetch patch` support via run-phase `FETCH_DIR/patches` mounts.
